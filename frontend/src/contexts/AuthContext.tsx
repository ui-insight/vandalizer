...
interface AuthContextValue {
  ...
  register: (userId: string, email: string, password: string, name?: string, inviteToken?: string, roleSegment?: string) => Promise<void>
  logout: () => Promise<void>
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const register = useCallback(
    async (userId: string, email: string, password: string, name?: string, inviteToken?: string, roleSegment?: string) => {
      const u = await authApi.register(userId, email, password, name, inviteToken, roleSegment)
      localStorage.removeItem('workspace:mode')
      setUser(u)
    },
    [],
