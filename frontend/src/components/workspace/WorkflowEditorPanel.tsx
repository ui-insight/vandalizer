...
function ValidateTab({
  ...
  // Build + download a comprehensive per-document validation report so the user
  // can diagnose exactly where/why a document failed (expected vs. finding +
  // the full workflow output).
  const downloadDocReport = (d: import('../../api/workflows').BatchValidationDocument) => {
    const planById = new Map(planChecks.map(c => [c.id, c]))
    const pct = (v?: number) => typeof v === 'number' ? `${Math.round(v * 100)}%` : 'n/a'
    const lines: string[] = [
      `# Validation Report — ${d.document_title}`,
      '',
      `- **Grade:** ${d.grade} (score ${d.score}/100)`,
      `- **Checks:** ${d.num_passed} passed / ${d.num_failed} failed`,
    ...
    for (const c of d.checks) {
      ...
      lines.push('')
      lines.push(`### [${c.status}] ${c.name}${cat} — confidence ${pct(c.consistency)}`)
      if (pc?.description) { lines.push(`**Expected:** ${pc.description}`); lines.push('') }
      lines.push(`**Finding:** ${c.detail || '—'}`)
    }
    lines.push('')
    lines.push('## Workflow Output (deliverable that was graded)')
    lines.push('')
    lines.push(d.output || '_(output not captured)_')
    lines.push('')
    lines.push('---')
    lines.push('_Generated by Vandalizer batch validation — for the RA\'s internal review._')

    const slug = (d.document_title || 'document').replace(/[^A-Za-z0-9._-]+/g, '-').replace(/^-+|-+$/g, '').toLowerCase() || 'document'
    const blob = new Blob([lines.join('\n')], { type: 'text/markdown;charset=utf-8' })
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = url
    a.download = `${slug}-validation.md`
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
              <thead>
                <tr style={{ textAlign: 'left', color: '#6b7280' }}>
                  ...
                  <th style={{ padding: '6px 8px', borderBottom: '1px solid #e5e7eb' }}>Results</th>
                </tr>
              </thead>
              <tbody>
                    ...
                    <tr key={d.session_id} style={{ borderBottom: '1px solid #f3f4f6' }}>
                      ...
                      <td style={{ padding: '6px 8px' }}>
                        <button
                          ...
                          title="Download this document's full validation report (Markdown)"
                          style={{
                            display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
                            width: 26, height: 26, borderRadius: 5, border: '1px solid #d1d5db',
                            backgroundColor: '#fff', color: '#374151', cursor: 'pointer',
                          }}
                        >
