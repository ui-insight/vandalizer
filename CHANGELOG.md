...
- **KB test-query generation surfaced transient LLM blips as 502s.** The synchronous generation path now retries connection/timeout/transport errors (up to 3 attempts, linear backoff), matching the Celery path; genuine 4xx/5xx provider errors still fail fast.
