...
- **Every answer is validated.** Extraction results carry a `QualityBadge` with tier, accuracy, consistency, and active alerts. The model never sees the number — it comes from stored `ValidationRun` records and is stripped before the payload reaches the LLM, so the agent cannot inflate its own grade.
