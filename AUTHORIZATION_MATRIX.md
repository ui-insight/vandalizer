...
| Workflows | owner; team member for team workflows; examiners / admins while a verification request is open | owner; team `owner` / `admin` | All CRUD and execution delegated through `get_authorized_workflow`. Document selections pass through `_authorize_documents`. Steps/tasks resolve parent workflow for authz. A third level, `validate`, covers the validation plan, test inputs, expected outputs and starting/cancelling an optimization run: everyone who can manage, plus examiners / admins while a verification request is open (they grade a submission without editing it). Applying or reverting an optimization winner stays at `manage`. | Audited |
