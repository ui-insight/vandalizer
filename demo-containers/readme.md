...
It relies on two environment variables:
...
The compose file pulls containers from Github Container Registry. It does not need the repository cloned, but you do need to have GHCR permissions on the `ui-insight/vandalizer` repository in your docker login.
...
To avoid conflicting with other Docker networks on the host, this is set to explicitly reserve IPs for each pod in `10.20`. Change or remove that setting in the network if necessary.
