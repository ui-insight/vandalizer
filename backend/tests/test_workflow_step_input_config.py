...
class TestEveryPathThatBuildsStepsDataResolvesInput:
    ...
    There is one place that assembles steps_data -- `build_steps_data` -- and
    the scheduled / folder-watch path and the optimizer both call it (#862).
    They used to keep copies of the loop, which is how the scheduled path
    missed this fix and the workflow default model (#842) in turn. The
    ordering is pinned once, on the builder; the delegation is pinned in
    test_steps_data_single_builder.
    """
    ...
    def test_the_celery_run_path_still_applies_it_before_preloading(self):
        ...
        src = inspect.getsource(workflow_tasks.build_steps_data)
