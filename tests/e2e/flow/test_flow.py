...
class TestLogin:
    def test_vandalizer_dev_login(self, driver, config) -> None:
...
class TestNavigation:
    def test_loads_home_page(self, driver, config) -> None:
        driver.get(config["home_url"])
        WebDriverWait(driver, 10).until(EC.url_matches(config["home_url"]))
        assert driver.title == "Home | Vandalizer"

    def test_enters_directory(self, driver, config) -> None:
        WebDriverWait(driver, 5).until(
    ...
    def test_enters_file(self, driver, config) -> None:
        WebDriverWait(driver, 5).until(
            EC.presence_of_element_located(
