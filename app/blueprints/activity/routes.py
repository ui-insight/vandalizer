...
from flask import Blueprint, render_template, request, current_app, jsonify, redirect, url_for, Response
...
def delete_activity(activity_id):
    ...
    if event:
        ...
        conversation = ChatConversation.objects(uuid=event.conversation_id, user_id=user_id).first()
        if conversation:
...
def generate_events(user_id, limit=100, poll_interval=2):
    ...
    while True:
        ...
        events = list(
            ActivityEvent.objects(user_id=user_id)
            .order_by("-started_at")
            .limit(limit)
        )
