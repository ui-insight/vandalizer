...
def ocr_extract_text_from_pdf(pdf_path: str, document=None) -> str:
    """
    ...
    #     contains_images = True if img_refs != [] else False
    #     if contains_images:
    #         break

    # if len(all_text) < MIN_PDF_TEXT_LENGTH or contains_images:
...
def extract_text_from_doc(doc_path, doc=None):
    if doc and len(doc.raw_text) > 100:
        return doc.raw_text
