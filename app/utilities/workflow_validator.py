...
class GeneratedCheck(BaseModel):
    """Single check generated by the LLM planner."""
    ...
    description: str
    severity: Severity = Severity.SHOULD
    weight: float = 1.0
...
class GeneratedPlan(BaseModel):
    """Structured output from the plan generation LLM call."""

    checks: list[GeneratedCheck]


class CheckVerdict(BaseModel):
    """Structured output from an LLM-based check evaluation."""

    status: str = Field(description="PASS, FAIL, WARN, or NEEDS_INVESTIGATION")
...
Evaluate the check and return a structured verdict:
- status: PASS if the check is fully satisfied, FAIL if clearly not met,
  WARN if partially met or minor issue, NEEDS_INVESTIGATION if uncertain.
- confidence: 0.0 to 1.0 indicating your confidence in the verdict.
- evidence: Quote or reference the specific part of the output that supports your verdict.
- reasoning: Brief explanation of why you chose this status.
- fix_suggestion: If FAIL or WARN, suggest how to fix the issue. Empty string if PASS.
...
class PlanGenerator:
    ...
    def generate(
        ...
        debug(f"LLM generated {len(generated_plan.checks)} checks")

        # Convert to dicts
        checks = [check.model_dump(mode="json") for check in generated_plan.checks]
...
class CheckRunner:
    ...
    def _run_llm_check(
        ...
        agent = Agent(
            model,
            system_prompt=CHECK_EVALUATION_SYSTEM_PROMPT,
        ...
        # Normalize status
        status = verdict.status.upper().strip()
        ...
        return {
            ...
            "fix_suggestion": verdict.fix_suggestion or "",
        }
