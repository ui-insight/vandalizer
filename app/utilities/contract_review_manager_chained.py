...
class ContractReviewManagerChained:
    ...
    def __init__(self):
        ...
        self.review_vector_db = Chroma(embedding_function=self.embeddings,
                                 persist_directory="./review_vectordb")
    
    def extract_sections(self, document):
        vectordb = self.load_contract(document)
        llm = ChatOpenAI(model_name="gpt-3.5-turbo", openai_api_key="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")
        retreiver = vectordb.as_retriever(search_kwargs={'k': 1})
        pdf_qa = ConversationalRetrievalChain.from_llm(llm, retreiver)

        result = pdf_qa({"question": "Give me a list of all sections in the contract, formatted as a csv with a comma between each section.", "chat_history": ""})
        print(result["answer"])
    ...
    def prepare_splits(self, document):
        ...
        text = "Given the following text: " + doc_string + ". Give me a list of all sections in the contract, formatted as a csv with a comma between each section."
