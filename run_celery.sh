...
start_queue_workers() {
    local queue_name=$1
    local num_workers=$(get_queue_info "$queue_name" "workers")
    local concurrency=$(get_queue_info "$queue_name" "concurrency")
    local pool_type=$(get_queue_info "$queue_name" "pool")
...
stop_flower() {
    ...
    local flower_pids=$(pgrep -f "celery.*flower")
    if [ -n "$flower_pids" ]; then
        echo $flower_pids | xargs kill -TERM 2>/dev/null
        sleep 1
        flower_pids=$(pgrep -f "celery.*flower")
        if [ -n "$flower_pids" ]; then
            echo $flower_pids | xargs kill -KILL 2>/dev/null
        fi
    fi
}
...
check_status() {
    ...
    for queue in $(get_all_queues); do
        local num_workers=$(get_queue_info "$queue" "workers")
...
case "${1:-start}" in
    ...
    logs)
        if [ -z "$2" ]; then
            ...
            # Check if it's a queue name or specific worker
            if [[ "$2" == *"_"* ]]; then
                # Specific worker format: queue_number
                local log_file="$LOG_DIR/worker_$2.log"
                if [ -f "$log_file" ]; then
